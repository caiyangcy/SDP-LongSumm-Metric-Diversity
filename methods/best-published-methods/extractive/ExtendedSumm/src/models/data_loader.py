...
def load_dataset(args, corpus_type, shuffle):
    ...
    if corpus_type == 'val' or corpus_type=='test':
        pts = sorted(glob.glob(args.bert_data_path + f'{corpus_type}_bert_files/' + corpus_type + '.*.pt'), reverse=True)
    ...
    if pts:
        ...
        pt = args.bert_data_path + f'processed/{corpus_type}_bert_files/' + corpus_type + '.0' + '.pt'
        yield _lazy_dataset_loader(pt, corpus_type)
...
def load_dataset_abs_bart(args, corpus_type, shuffle):
    ...
    if corpus_type == 'val' or corpus_type=='test':
        pts = sorted(glob.glob(args.bert_data_path + f'processed/{corpus_type}_bert_files/' + corpus_type + '.*.pt'), reverse=True)
        pts = [(int(f.split('.')[-3]), f) for f in pts]
        pts = sorted(pts, key=lambda tup: tup[0], reverse=False)
        pts = [p[1] for p in pts]
