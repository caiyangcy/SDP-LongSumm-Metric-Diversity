...
class Trainer(object):
            ...
            for i, batch in enumerate(train_iter):
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    if accum == self.grad_accum_count:
                        ...
                        if (step == 5 or step % self.args.val_interval == 0) and valid_iter_fct is not None:  # Validation
    ...
    def validate_rouge_baseline(self, valid_iter_fct, step=0, valid_gl_stats=None, write_scores_to_pickle=False):
        ...
        preds_with_idx = {}
        golds = {}
        can_path = '%s%s_source.txt' % (self.args.result_path_test, self.args.exp_set)
        gold_path = '%s%s_target.txt' % (self.args.result_path_test, self.args.exp_set)
        # score_path = '%s%s_probs.txt' % (self.args.result_path_test, self.args.exp_set)

        save_pred = open(can_path, 'w')
        save_gold = open(gold_path, 'w')
        # save_prob = open(score_path, 'w')
        sent_scores_whole = {}
        ...
        for p_idx, (p_id, sent_scores) in enumerate(sent_scores_whole.items()):
            ...
            # for i in selected_ids_unsorted:
            #     i_prev = max(0, i-1)
            #     i_seq = min(0, len(p_src)-1)
            #     if i_prev not in selected_ids_expanded:
            #         selected_ids_expanded.append(i_prev)
            #     selected_ids_expanded.append(i)
            #     if i_seq not in selected_ids_expanded:
            #         selected_ids_expanded.append(i_seq)
            # selected_sents_scores = []

            # _pred = []
            # for j in selected_ids_expanded: #selected_ids_unsorted:
            #     if (j >= len(p_src)):
            #         continue
            ...
            #         # selected_sents_scores.append( sent_scores[j] )

            #     if (len(_pred) == PRED_LEN):
            #         break
            # _pred = sorted(_pred, key=lambda x: x[1])
            # # print("selected_sents_scores: ", selected_sents_scores)

            # # sent_scores_to_write.append( np.prod( selected_sents_scores ) )
        ...
        for cnt, (id, pred) in enumerate(preds.items()):
            pred = pred.encode('utf-8', 'ignore').decode('utf-8')
            save_pred.write(pred.strip().replace('<q>', ' ') + '\n')
            
            if cnt == len(preds)-1:
                print(f"cnt: {cnt} match pred length: {len(pred)-1}")
                save_gold.write(golds[id].replace('<q>', ' ').strip() )
            else:
                save_gold.write(golds[id].replace('<q>', ' ').strip() + '\n\n')

            # save_prob.write( str(sent_scores_to_write[cnt]) + "\n" )
