...
from train_extractive import train_ext, validate_ext, test_ext, test_ext_abs
...
if __name__ == '__main__':
    ...
    parser.add_argument("-result_path", default='../results/longsumm')
    ...
    if not os.path.exists(args.result_path_test): 
        os.makedirs(args.result_path_test)
    ...
    if (args.task == 'abs'):
        ...
        elif (args.mode == 'test-abs'):
            cp = args.test_from
            try:
                step = int(cp.split('.')[-2].split('_')[-1])
            except:
                step = 0
            test_ext_abs(args, device_id, cp, step, args.section_prediction)
        elif (args.mode == 'test_text'):
            cp = args.test_from
            try:
                step = int(cp.split('.')[-2].split('_')[-1])
