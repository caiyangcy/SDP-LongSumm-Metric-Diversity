...
from models.data_loader import load_dataset, load_dataset_abs_bart
...
def test_ext_abs(args, device_id, pt, step, is_joint=False):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
    ...
    def test_iter_fct():
        return data_loader.Dataloader(args, load_dataset_abs_bart(args, args.exp_set, shuffle=False), args.test_batch_size, device,
                                      shuffle=False, is_test=True)
