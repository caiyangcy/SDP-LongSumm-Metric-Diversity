...
class Evaluator(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if  eva > self.best_metric:  # 保存最优
            ...
            if not os.path.exists(f'../../replication/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{self.idx}'):
                os.mkdir(f'../../replication/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{self.idx}')
            self.model.save_weights(f'../../replication/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{self.idx}/extract_model_{self.fold}.hdf5')
...
def main(idx):
    ...
    data_x = np.load(f'../../datasets/dataset_multiple_splits/split_{idx}/LongSumm2021/abstractive_test_for_extractive_prediction/docs_roberta_avail.npy')
    
    # data_y = np.zeros( (token_x.shape[0], token_x.shape[1], 1)  )
    data_y = np.zeros_like(data_x[..., :1])

    with open(f"../../datasets/dataset_multiple_splits/split_{idx}/LongSumm2021/abstractive_test_for_extractive_prediction/labels_avail.pickle", 'rb') as handle:
        labels = pickle.load(handle)

        
    with open(f"../../datasets/dataset_multiple_splits/split_{idx}/LongSumm2021/abstractive_test_for_extractive_prediction/data_avail.pickle", 'rb') as handle:
    ...
    for fold in range(num_folds):
        ...
        model.load_weights(f'../../replication/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{idx}/extract_model_{fold}.hdf5')
    ...
    if not os.path.exists(f"../../leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/"):
        os.mkdir(f"../../leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/")

    with open(f"../../leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/system_{threshold_to_write}_trunc.txt", "w") as f:
        pred_to_write = "\n".join( predictions_trunc )
        f.write(  pred_to_write   )

    with open(f"../../leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/system_{threshold_to_write}_full.txt", "w") as f:
        pred_to_write = "\n".join( predictions_full )
        f.write(  pred_to_write   )

    with open(f"../../leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/reference_{threshold_to_write}.txt", "w") as f:
        truth_to_write = "\n\n".join( groudtruth )
        f.write(  truth_to_write   )

    with open(f"../../leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/system_probs_{threshold_to_write}.txt", "w") as f:
