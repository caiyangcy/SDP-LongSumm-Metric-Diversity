...
class Evaluator(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if  eva > self.best_metric:  # 保存最优
            ...
            if not os.path.exists(f'REPLACE-BY-YOUR-PATH/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{self.idx}'):
                os.mkdir(f'REPLACE-BY-YOUR-PATH/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{self.idx}')
            self.model.save_weights(f'REPLACE-BY-YOUR-PATH/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{self.idx}/extract_model_{self.fold}.hdf5')
...
def main(idx):
    ...
    data_x = np.load(f'REPLACE-BY-YOUR-PATH/datasets/dataset_multiple_splits/split_{idx}/LongSumm2021/abstractive_test_for_extractive_prediction/docs_roberta_avail.npy')
    
    # data_y = np.zeros( (token_x.shape[0], token_x.shape[1], 1)  )
    data_y = np.zeros_like(data_x[..., :1])

    with open(f"REPLACE-BY-YOUR-PATH/datasets/dataset_multiple_splits/split_{idx}/LongSumm2021/abstractive_test_for_extractive_prediction/labels_avail.pickle", 'rb') as handle:
        labels = pickle.load(handle)

        
    with open(f"REPLACE-BY-YOUR-PATH/datasets/dataset_multiple_splits/split_{idx}/LongSumm2021/abstractive_test_for_extractive_prediction/data_avail.pickle", 'rb') as handle:
    ...
    for fold in range(num_folds):
        ...
        model.load_weights(f'REPLACE-BY-YOUR-PATH/Longsumm_code/model/DGCNN_saved_splits_train_on_abs/split_{idx}/extract_model_{fold}.hdf5')
        ...
        for d, yp in tqdm(zip(test_data, pred), desc='evaluating'):
            ...
            predictions_trunc.append(pred_sum.strip())
            predictions_full.append(pred_full.strip())
            groudtruth.append(d[1].strip())


    threshold_to_write = int(threshold*100)

    if not os.path.exists(f"REPLACE-BY-YOUR-PATH/leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/"):
        os.mkdir(f"REPLACE-BY-YOUR-PATH/leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/")

    with open(f"REPLACE-BY-YOUR-PATH/leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/system_{threshold_to_write}_trunc.txt", "w") as f:
        pred_to_write = "\n".join( predictions_trunc )
        f.write(  pred_to_write   )

    with open(f"REPLACE-BY-YOUR-PATH/leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/system_{threshold_to_write}_full.txt", "w") as f:
        pred_to_write = "\n".join( predictions_full )
        f.write(  pred_to_write   )

    with open(f"REPLACE-BY-YOUR-PATH/leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/reference_{threshold_to_write}.txt", "w") as f:
        truth_to_write = "\n\n".join( groudtruth )
        f.write(  truth_to_write   )

    with open(f"REPLACE-BY-YOUR-PATH/leaderboard_splits/split_{idx}/baseline/DGCNN_train_on_abs/system_probs_{threshold_to_write}.txt", "w") as f:
